...
def apply_theme(t):
    st.markdown(f"""
    <style>
        /* Main output area changes with theme */
        .stApp {{ background-color: {t['bg']} !important; }}

        /* Sidebar ALWAYS stays dark — never changes */
        ...
        /* Output area cards use theme colors */
        .cf-card {{
            background: {t['card']};
        ...
        /* Generate button uses theme accent */
        ...
        /* Main content text follows theme */
...
# ── Sidebar ──────────────────────────────────────────────────
with st.sidebar:
    st.markdown(f"<h2 style='margin:0'>✨ CreativeFlow AI</h2>", unsafe_allow_html=True)
    ...
    genre = st.selectbox("📚 Genre", ["Sci-fi","Fantasy","Romance","Thriller","Historical","Horror"])
    ...
    <div style='text-align:center'>
        ...
        <a href='{pinterest_url}' target='_blank' class='cf-pinterest-btn'>
            📌 Search Pinterest
        </a>
    </div>
    """, unsafe_allow_html=True)
...
if generate_btn:
    if not prompt:
        ...
        if gen_music:
            progress.progress(60, text="🎵 Composing music direction...")
            music_result = generate_music(prompt, mood, genre, knowledge)
        if gen_visual:
        ...
        st.session_state.update({
            'story_result': story_result, 'music_result': music_result,
            'visual_result': visual_result, 'prompt': prompt,
            'mood': mood, 'genre': genre, 'theme': theme
        })
...
if 'story_result' in st.session_state:
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["📖 Story","🎵 Music","🎨 Visual","📌 Inspiration","📤 Export"])
    ...
    with tab1:
        if story_result:
            ...
            c3.metric("Genre", st.session_state.get('genre',''))
            st.markdown("**🚀 Continue your story:**")
            st.markdown(f"""
            <a href='https://veo.google.com' target='_blank' class='cf-platform-btn'>🎬 Veo 3 — Make into Video</a>
    ...
    with tab2:
        if music_result:
            st.markdown(f"<div class='cf-card'><h2>{music_result['song_title']}</h2><p style='opacity:.7'>{music_result['genre_style']}</p></div>", unsafe_allow_html=True)
            c1,c2,c3 = st.columns(3)
            c1.metric("Tempo", music_result['tempo'])
            c2.metric("Chords", music_result['chord_progression'])
            c3.metric("Instruments", len(music_result['instruments']))
            st.markdown("**🎼 Lyrics:**")
            st.code(music_result['lyrics'], language=None)
            st.markdown(f"**🎛️ Production Notes:** {music_result['production_notes']}")
            st.markdown("**🚀 Continue your music:**")
            st.markdown(f"""
    ...
    with tab3:
        if visual_result:
            ...
            <a href='https://veo.google.com' target='_blank' class='cf-platform-btn'>🎬 Veo 3</a>
            """, unsafe_allow_html=True)
    ...
    with tab4:
        ...
        with c1:
            ...
            <div class='cf-card' style='text-align:center'>
                ...
                <p style='font-size:13px;opacity:.6'>{saved_mood} × {saved_genre} concept art, color palettes and mood boards</p>
        ...
        with c2:
            ...
            <div class='cf-card' style='text-align:center'>
                ...
                <p style='font-size:13px;opacity:.6'>Visual references matching your specific scene concept</p>
    ...
    with tab5:
        ...
        with c1:
            ...
            if music_result: st.write(f"✅ Music: *{music_result['song_title']}*")
        ...
        with c2:
            ...
            st.write(f"Mood × Genre: {st.session_state.get('mood','')} × {st.session_state.get('genre','')}")
