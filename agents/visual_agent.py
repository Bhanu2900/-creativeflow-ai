...
def generate_visual(
    ...
    art_style = visual_info.get(
        "art_style",
        "Digital Art"
    )

    color_palette = visual_info.get(
        "color_palette",
        ["Blue", "Black", "Silver"]
    )

    lighting = visual_info.get(
        "lighting",
        "Dramatic"
    )

    visual_references = visual_info.get(
        "visual_references",
        ["Cinematic", "Epic"]
    )

    full_prompt = f"""
You are a world-class visual art director and concept artist.

Based on this creative idea:
{prompt}
...
SCENE_DESCRIPTION:
[2-3 sentences describing the main scene]

COLOR_STORY:
[describe the color mood and palette in 2 sentences]
...
IMAGE_PROMPT:
[a detailed single paragraph prompt that could be used with an AI image generator]
"""
